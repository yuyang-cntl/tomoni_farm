class Farmer::StatisticsController < ApplicationController
  def index
    # 商品ごとの月別売上
    @stacked_sales = Item.where(farmer_id: current_farmer.id).map do |item|
    ...
    item_ids = Item.where(farmer_id: current_farmer.id).pluck(:id)
    ...
    # 投稿に対するいいね数
    @stacked_likes = Post.joins(:diary).where(farmer_id: current_farmer.id).map do |post|
